...
\begin{document}
\title{Poisson Tail Formation in Retention Loss of Cryogenic Condition of 3-D NAND Flash Memories by Electron Tunneling Emission}
...
\begin{abstract}
3D NAND Flash memory's operation of program and read holds pass voltage to make the switch-on for untarget page under the operation.
Due to the voltage that is lower than program voltage, the untarget cell can avoid from the program operation such as a target cell.
However, the pass voltage still could cause the difference to the $\Delta V_\text{T}$ to the untarget pages' cell over long time over total operation time (miliseconds).
The study examine about the  
\end{abstract}
...
In this study, the retention loss by tunneling emission of trapped electrons from the the programmed cells in various PV levels. By using the calibrated TCAD simulation to previous studies, the loss schematic in Poisson distribution will be shown thorught this paper.
